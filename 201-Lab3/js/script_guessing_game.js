'use strict';
var questions = ['Do I live in Hyderabad?',
  'Do I work for Amazon?',
  'Do I like Seattle?',
  'Do I like Java language?',
  'Do I like Pizza?',
...
for (i = 0; i < questions.length; i++) {
  ...
  if (guess === null) {
    ...
    if (checkCharacterAnswer(guess,answers[i])) {
      alert('Correct');
      correct++;
    }
    else {
    ...
    while (attemptsLeft >= 0) {
      ...
      if (checkNumberAnswer (guess,answers[i])) {
    ...
    for (j = 0; j < answers[i].length; j++) {
      if (checkCharacterAnswer (guess, answers[i][j])) {
        alert('You have guessed it correctly.');
        correct++;
        break;
      }
...
function checkCharacterAnswer (guess, answer) {
...
function checkNumberAnswer (guess, answer) {
